(function() {
    ...
    app.controller('ChatsDetailCtrl', function($scope, $stateParams, Chats, $ionicScrollDelegate,$ionicActionSheet,$timeout) {
        ...
        function createMessage (msg) {
            ...
            return angular.extend({}, nextMessage)
        }
        ...
        $scope.add = function() {
            $scope.messages.push(createMessage ($scope.message));
        ...
        $scope.doRefresh = function() {
            for (var i = 0, m = Math.floor(Math.random()*15); i < m; i++) {
                $scope.messages.unshift({content: Math.random()})
            };
